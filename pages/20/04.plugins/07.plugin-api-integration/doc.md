...
4. If `component` is set, the Admin2 frontend loads the plugin's web component via `GET /gpm/plugins/{slug}/report-script/{reportId}`
...
When `component` is `null`, the Admin2 frontend renders items using a built-in default renderer. When set, the frontend loads your custom web component.
