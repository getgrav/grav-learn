...
There are cases were you'd want to add a change a plugin's provided blueprint; to add, move, or delete the options there. This isn't straightforward: A plugin's blueprint contains more than just a `form`-property, and isn't implicitly declared as extendable. However, when building plugins it is worthwile to faciliate this for your [user's blueprints](/20/basics/folder-structure#user-blueprints).
