...
All endpoints except `/auth/token` and `/auth/refresh` require authentication via API Key, JWT access token (sent as `X-API-Token` or `Authorization: Bearer`), or active session. See [Authentication](/20/api/authentication).
