...
Adding REST API endpoints to a Grav 2.0 plugin via the first-party [Grav API plugin](/2/api).
...
6. Once it works, add `'2.0'` to the [`compatibility:` flag](/2/plugins/plugin-compatibility) in `blueprints.yaml`, bump the version, and release.
...
These skills do not replace the [API](/2/api), [plugin compatibility](/2/plugins/plugin-compatibility), or [admin translations](/2/plugins/admin-translations) reference docs. They tell an agent which patterns to apply and which mistakes to avoid; the reference docs are the source of truth for what the patterns are. If you're doing the porting work by hand, work from the reference docs. If you're handing the work to an agent, install the skills so the agent has the same context.
