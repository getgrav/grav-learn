...
The wizard checks each installed plugin's [`compatibility:` blueprint flag](/2/plugins/plugin-compatibility) for `2.0`. A plugin is treated as incompatible if it lists `1.7` only, or if it has no flag and its `dependencies:` declares `grav >= 1.7`.
...
The full guide, including the recommended alternatives (move the logic into a page template or a shortcode), is on the [Twig in Content](/2/content/twig-in-content) page. This affects page **content** only; theme and plugin `.html.twig` files are trusted and never sandboxed.
