...
- **Plugins and themes**: handled through the [`compatibility:` blueprint property](/2/plugins/plugin-compatibility). 2.0-flagged packages are pulled fresh from GPM; everything else is either skipped or installed disabled, depending on the policy you choose.
...
- **[Plugin Compatibility](/2/plugins/plugin-compatibility)**: declaring the `compatibility:` flag in your `blueprints.yaml` so GPM and the migration wizard will install your package on 2.0.
- **[Plugin API Integration](/2/plugins/plugin-api-integration)**: exposing your plugin's functionality through the [Grav API plugin](/2/api) so Admin 2.0 and external clients can use it.
- **[Admin Translations](/2/plugins/admin-translations)**: adapting your `languages/*.yaml` so labels and help text render correctly in Admin 2.0 (including the optional `ICU.` namespace for plurals and placeholders).
