...
> [!NOTE]
> Credentials are YAML-only by design. Keep them out of version control, for example by putting the `storage` block in an [environment-specific config](/2/advanced/environment-config) or by using Grav's config alias and `.env` support.
