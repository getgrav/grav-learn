...
- The **admin** method installs it from the Plugins screen in [Admin Next](/2/admin-panel).
...
In [Admin Next](/2/admin-panel), go to **Plugins** in the sidebar and click **Add**. Search for the plugin, then click its **Install** button. Dependencies are handled the same way as on the command line, and you will be asked to confirm them.
...
> [!WARNING]
> A plugin written for Grav 1.x will not necessarily work on Grav 2.0, and one that hooks into the classic admin panel definitely will not. See [Plugin Compatibility](/2/plugins/plugin-compatibility) for what changed and how to tell.
