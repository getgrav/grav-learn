...
The preferred method for keeping Grav, Plugins and Themes up to date is to use the **Grav Package Manager (GPM)**. Full information can be found in the [Grav GPM Documentation](/2/advanced/grav-gpm).

We also have **GPM** integrated into our [Administration Panel](/2/admin-panel) plugin which will check, prompt, and automatically install any updates.
...
See the [Migration](/2/migration) chapter for the full process, including:

- The [Assisted Migration](/2/migration/assisted-migration) workflow via the Migrate to Grav 2.0 plugin
- The [Manual Migration](/2/migration/manual-migration) procedure for complex setups
- The [Developer Upgrade Guide](/2/migration/developer-upgrade-guide) for plugin and theme authors
...
Plugin authors declare compatibility in their `blueprints.yaml` using the `compatibility:` property. See [Plugin Compatibility](/2/plugins/plugin-compatibility) for full details.
...
> [!NOTE]
> **TIP:** More information about the command can be found from [GPM Command > Self-upgrade](/2/cli-console/grav-cli-gpm#self-upgrade).
...
> [!NOTE]
> **TIP:** More information about the command can be found from [GPM Command > Update](/2/cli-console/grav-cli-gpm#update).
