...
All Grav configuration files are written in [YAML syntax](/2/advanced/yaml) with a `.yaml` file extension.  YAML is very intuitive which makes it very easy to both read and write, however, you can check out the [YAML page in the Advanced chapter](/2/advanced/yaml) to get a complete understanding of the syntax available.

> [!NOTE]
> **TIP:** See [Security > Configuration](/2/security/configuration) for a quick guide how to secure and optimize your production site.
...
| ... **twig:** | Enable or disable the processing of twig on the front end. Can be set `true` or `false`. **Grav 2.0:** Twig in page **content** is also gated by a security switch and runs inside a sandbox; this `process.twig` flag alone is no longer enough to render content Twig. See [Twig in Content](/2/content/twig-in-content) |
...
| **markdown_output:** | Serve any page as Markdown for AI agents, at `<route>.md` or on `Accept: text/markdown`. See [Markdown for AI Agents](/2/content/markdown-output) (**Grav 2.1+**) |
...
| **check:** | |
| ... **method:** | Method to check for updates in pages. Options: `file`, `folder`, `hash` and `none`. [more details](/2/advanced/performance-and-caching#grav-core-caching) |
...
| **collections:** | This contains collections, designated as sub-items. For example: `jquery: system://assets/jquery/jquery-3.x.min.js`. [Read more about this](/2/themes/asset-manager#collections-and-attributes) |
[/div]
...
| **token:** | Shared secret a remote client must send to read `/__clockwork/` profiler data. Empty means the data can only be read from the server itself. See [Debugging](/2/advanced/debugging#reading-profiler-data-from-a-remote-site) (**Grav 2.0.22+**) |
| **shutdown:** | |
...
| **metadata:** | Set default metadata for all your pages, see the [content page headers](/2/content/headers) section for more details |
| **summary:** | |
...
> [!WARNING]
> You can also provide a custom blueprint to make your custom file editable in the admin. Check out the relevant [recipe in the Admin Cookbook section](/2/cookbook/admin-recipes#add-a-custom-yaml-file).
