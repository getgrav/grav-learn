...
Grav also needs a handful of PHP extensions, and `zip` in particular is used by both Composer and GPM. See the [requirements](/2/basics/requirements) page for the full list before you go any further.

## Option 1: Install from ZIP package

The easiest way to install Grav is to download the ZIP package and extract it:

1. Download the latest-and-greatest **[Grav](https://getgrav.org/download/core/grav/latest)** or **[Grav + Admin](https://getgrav.org/download/core/grav-admin/latest)** package. **Grav + Admin** is core with the [API](/2/api) and [Admin Next](/2/admin-panel) plugins already bundled, so it gives you a working admin panel with nothing else to install.
...
3. Install the **plugin** and **theme dependencies** by using the [Grav CLI application](/2/advanced/grav-cli) `bin/grav`:
...
Otherwise, install [Admin Next](/2/admin-panel) with a single GPM command from the root of your Grav install:
...
Admin Next depends on the [API plugin](/2/api), which in turn depends on Login, Form, Email, and Shortcode Core. GPM resolves that whole chain for you and prompts once to install the dependencies, so `bin/gpm install admin2` is all you need to type.
...
Please consult the [Troubleshooting](/2/troubleshooting) section for help regarding specific issues.

> [!WARNING]
> If you have issues with file permissions, please check the [Permissions Troubleshooting documentation](/2/troubleshooting/permissions). Also, you could look at the [Hosting Guides documentation](/2/webservers-hosting) that has specific instructions for various hosting environments

## Grav Updates

To keep your site up to date, please read [Updating Grav & Plugins](/2/basics/updates).
