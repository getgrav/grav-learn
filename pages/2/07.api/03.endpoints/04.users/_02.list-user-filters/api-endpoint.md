...
A plugin filter can only *narrow* the listing: it runs after `search`, and the `access` and `group` filters and pagination are applied by the core endpoint after it, so a tab can never widen visibility or change the response envelope. Tabs work with both account backends. With Flex accounts the event's `collection` is a Flex collection; with plain file accounts it is an `ArrayCollection` of users keyed by username, and only accounts already in the list are kept from whatever the plugin hands back. See the [API Events](/2/api/events) page for the `onApiUserListFilters` and `onApiUserListFilter` contracts.
