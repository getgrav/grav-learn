...
`crontab_status` is one of: `not_installed`, `installed`, `unknown`. `cron_detection` names how that was worked out, or `unavailable` when detection failed. `process_available` is false when PHP's `proc_open` is disabled, in which case `POST /scheduler/run` returns 501. `last_run` is the last run by a real trigger (cron or webhook); `last_manual_run` is the last run started from the admin or the API, which never counts as proof that cron is working.
