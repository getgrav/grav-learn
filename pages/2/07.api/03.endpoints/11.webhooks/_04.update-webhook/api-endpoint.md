...
api:
    method: PATCH
    path: '/webhooks/{id}'
    description: 'Partial update — only supplied fields are changed. `url` is re-validated; `events` are re-validated against the allowed event list. Supports [optimistic concurrency control](/2/api/getting-started#concurrency-control) via the `If-Match` header.'
    parameters:
        - name: id
          type: string
          required: true
