...
When your media carry metadata via `.meta.yaml` sidecars, you can filter and sort the listing by those values instead of pulling the whole set. This mirrors the [Twig collection query methods](/2/content/media#filtering-media-by-metadata) but is **bound to the metadata schema** configured under **Plugins → API → Media Metadata** (`plugins.api.media_metadata.fields`).
