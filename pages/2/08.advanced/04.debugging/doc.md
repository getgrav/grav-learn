...
Plugin code that runs on the API path, such as an [`onApi*` event handler](/2/api) or a controller extending `AbstractApiController`, **must not** call `dump()`. The API returns JSON, and `dump()` writes into that body, producing a broken response.
