...
> [!NOTE]
> We assume you already know how to build a form. If not, read **[Forms](/2/forms)** first, then come back here.
...
Together these three classes define the behavior of your type. To customize behavior beyond what blueprints allow, extend these classes and point the config at your own subclasses. See [PHP Classes](/2/advanced/flex/php-and-events) for how to register and use them.
...
This backs the `GET /flex-objects/{type}/export` endpoint (see the [REST API reference](/2/api/endpoints/flex-objects)).
...
`{TYPE}` defaults to the directory type and `{LAYOUT}` defaults to `default`. Both can be overridden at render time, for example by the `render` tag or the `[flex-objects]` shortcode. See [Frontend](/2/advanced/flex/frontend) for how these templates are used.
...
The `object.compat.events` toggle above is a real setting: it controls whether `onAdminSave` / `onAdminAfterSave` fire when objects are written, which older plugins depend on. See [Events](/2/advanced/flex/php-and-events) for the full list of events a plugin author can hook.
