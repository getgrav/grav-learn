...
The flex-objects page type lets a page act as a front controller for a directory (see [Frontend Display](/2/advanced/flex/frontend)). The mechanism behind it is a dynamically named event.
...
- [Storage & Configuration](/2/advanced/flex/custom-types/blueprint-reference#config-data) for the `config.data` block these classes plug into.
- [Permissions](/2/advanced/flex/custom-types/blueprint-reference#permissions) before you expose a plugin-registered directory over the REST API.
- [Frontend Display](/2/advanced/flex/frontend) for the page type that the routing event powers.
