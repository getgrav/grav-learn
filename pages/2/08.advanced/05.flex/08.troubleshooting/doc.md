...
See the [Blueprint Reference permissions section](/2/advanced/flex/custom-types/blueprint-reference#permissions) for the full permission block structure, and the [Flex Objects API docs](/2/api/endpoints/flex-objects) for how tokens and grants are checked per endpoint.
...
> [!TIP]
> When in doubt about which permission actions to grant or how a specific endpoint checks them, cross-reference the [Blueprint Reference permissions section](/2/advanced/flex/custom-types/blueprint-reference#permissions) and the [Flex Objects API docs](/2/api/endpoints/flex-objects). Getting the `api.<type>` grants right is what resolves the large majority of post-migration 403s.
