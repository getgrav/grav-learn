...
**XSS Injection Detection** is now enabled in all the frontend forms by default. Check the [documentation](/17/forms/forms/form-options#xss-checks) on how to disable or customize the checks per form and per field.
...
Admin has now new [Accounts Administration](/17/admin-panel/accounts) using **Flex Users**:
...
The existing [Pages Administration](/17/admin-panel/page) has been greatly improved with **Flex Pages**:
...
Added new configuration option to set fallback content languages individually for every language
  * system.yaml: `languages.content_fallback`: See [Language Configuration](/17/content/multi-language#language-configuration)
