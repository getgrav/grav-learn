...
Accessing the CLI is a simple process but you need to use a **terminal**.  On a mac this is called `Terminal`, on windows its called `cmd` and on Linux its just a shell (UNIX style commands are not available with the native Windows command prompt, installing [gitbash](http://msysgit.github.io/) allows you to run the same commands on a Windows machine).  If you are accessing your server remotely, you most likely will use **SSH** to remotely log in to your server.  Check out this [great tutorial for more information on SSH](http://code.tutsplus.com/tutorials/ssh-what-and-how--net-25138).
