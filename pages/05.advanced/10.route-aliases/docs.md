...
A more advanced type of alias redirect allows the use of a simple **wildcard** (`*`) to map part of an alias to a route.  For example, if you had:
...
With this route alias, any URL that confirms to the wildcard: `/one-ring/to-rule-them-all` or `/one-ring/is-mine` will both show the content from the page with the route `/blog/sunshine-in-the-hills`.
...
>>> There is no support for wildcard functionality in redirects currently. This is something we plan on adding in the future.
