...
All Grav configuration files are written in [YAML syntax](/18/advanced/yaml) with a `.yaml` file extension.  YAML is very intuitive which makes it very easy to both read and write, however, you can check out the [YAML page in the Advanced chapter](/18/advanced/yaml) to get a complete understanding of the syntax available.

> [!NOTE]
> **TIP:** See [Security > Configuration](/18/security/configuration) for a quick guide how to secure and optimize your production site.
...
| **check:** | |
| ... **method:** | Method to check for updates in pages. Options: `file`, `folder`, `hash` and `none`. [more details](/18/advanced/performance-and-caching#grav-core-caching) |
...
| **collections:** | This contains collections, designated as sub-items. For example: `jquery: system://assets/jquery/jquery-3.x.min.js`. [Read more about this](/18/themes/asset-manager#collections-and-attributes) |
[/div]
...
| **metadata:** | Set default metadata for all your pages, see the [content page headers](/18/content/headers) section for more details |
| **summary:** | |
...
> [!WARNING]
> You can also provide a custom blueprint to enable your custom file to be editable in the admin plugin. Check out the relevant [recipe in the Admin Cookbook section](/18/admin-panel/admin-recipes#add-a-custom-yaml-file).
