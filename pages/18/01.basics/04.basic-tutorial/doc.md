...
Assuming you successfully [installed Grav](/18/basics/installation) with the instructions listed in the previous chapter, we can continue and play around with Grav a little to get you more comfortable.
...
First, let us familiarize ourselves with where Grav stores content.  We will go into more depth in a [future chapter](/18/basics/folder-structure), but for the time being, you need to be aware that all our user content, is stored in the `user/pages/` folder of your Grav install.
...
You can read an [extensive write-up of available syntax](/18/content/markdown) with examples in the documentation, but for now, follow along.
...
Let us break this down a little so you can see how easy it is to write in Markdown. The stuff between the `---` indicators are the [Page Headers](/18/content/headers), and these are written in a straightforward format called [YAML](/18/advanced/yaml). This configuration block that sits in the `.md` file is commonly known as **YAML Front Matter**.
